---
name: remotion
description: "Remotion video creation best practices: build programmatic videos with React (animations, audio, captions, subtitles, 3D, charts, fonts, GIFs, Lottie, maps, transitions, text effects). Use when the user asks to create, edit, or render a video with Remotion, scaffold a Remotion project (npx create-video), add captions or audio, or debug Remotion code. 38 rule files under rules/ cover specific domains and should be loaded on demand. 中文：用 Remotion（React 编程式视频）新建项目、写/改合成、渲染视频、加字幕/音频时使用。"
compatibility: Requires Node.js >= 20 with npx and network access to install Remotion packages from the npm registry.
allowed-tools: Bash
---
