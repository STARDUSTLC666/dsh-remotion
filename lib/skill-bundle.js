...
function analyze(text) {
    ...
    if (!match)
        return { parsed, error: '缺少完整的 YAML frontmatter（--- 分隔行）。' };
    try {
        const doc = YAML.parseDocument(match[1], { prettyErrors: false, uniqueKeys: true });
        if (doc.errors.length || doc.warnings.length)
            throw new Error([...doc.errors, ...doc.warnings].map((error) => error.message).join('; '));
        const value = doc.toJS({ maxAliasCount: 100 });
        if (typeof value !== 'object' || value === null || Array.isArray(value))
            return { parsed, error: 'frontmatter 必须是 YAML 对象。' };
        const fields = value;
    ...
    if (!SKILL_NAME.test(parsed.name))
        return { parsed, error: 'name 必须为非空小写 kebab-case 名称。' };
    if (!parsed.description.trim())
        return { parsed, error: 'description 必须为非空字符串。' };
    if (!parsed.content.trim())
        return { parsed, error: '技能正文为空。' };
    return { parsed };
}
/** Preserve the original non-throwing public parsing helper and return shape. */
export function parseSkillFile(text) { return analyze(text).parsed; }
function inspect(directory, name) {
    ...
    try {
        ...
        return { check: { name, file, ok: false, code: 'unreadable', detail: 'SKILL.md 不可读取：' + message(error) } };
    }
    const result = analyze(text);
    const error = result.error ?? (result.parsed.name === name ? undefined : 'frontmatter name 与技能目录名称不一致：' + result.parsed.name);
    if (error)
        return { check: { name, file, ok: false, code: 'invalid', detail: error } };
    return {
...
export function createSkillBundle(options) {
    const names = [...options.names];
    if (new Set(names).size !== names.length || names.some((name) => !SKILL_NAME.test(name)))
        throw new Error('Skill bundle requires unique kebab-case directory names');
    ...
    return {
        ...
        checkFiles() { return inspectAll().map((result) => result.check); },
        apply(ctx) {
            ...
            for (const result of inspectAll()) {
                ...
                if (!result.parsed || !result.fingerprint) {
                    warn(result.check.name + '：' + result.check.detail);
                    continue;
                }
                const definition = {
                    ...result.parsed,
                ...
                try {
                    ...
                    if (typeof remove !== 'function')
                        throw new Error('skills.register 未返回卸载函数。');
            ...
            const health = async () => {
                ...
                for (const result of inspectAll()) {
                    ...
                    if (!disposed && registration.definition) {
                        if (typeof ctx.skills.get !== 'function')
                            registryError = '宿主未提供 skills.get，无法确认技能是否实际生效。';
                        else {
                            try {
                                ...
                                registryError = '读取宿主技能注册表失败：' + message(error);
                            }
                        }
                    }
                    let code = 'ready';
                    ...
                    if (disposed) {
                        ...
                        detail = '加载时注册失败：' + registration.error;
                        reloadRequired = true;
                    }
                    else if (!registration.definition) {
                        code = 'not_registered';
                        detail = '文件现已有效，但加载时未注册；请重载插件。';
                        reloadRequired = true;
                    }
                    else if (result.fingerprint !== registration.fingerprint) {
                        code = 'changed';
                        detail = '文件已变化，当前注册仍是加载时版本；请重载插件。';
                        reloadRequired = true;
                    }
                        ...
                        code = 'inactive';
                        detail = '宿主注册表中的技能缺失或已被其他内容替代；请重载插件。';
                        reloadRequired = true;
                    }
            ...
            if (typeof ctx.tools?.register === 'function') {
                try {
                        ...
                        output: {
                            ...
                            render(_args, value) {
                                ...
                                for (const skill of result.skills)
                                    lines.push('- ' + skill.name + '：' + (skill.ok ? '✅' : '❌ ' + skill.detail));
                    ...
                    if (typeof remove !== 'function')
                        throw new Error('tools.register 未返回卸载函数。');
                    disposers.push(remove);
                }
                catch (error) {
                    dispose();
